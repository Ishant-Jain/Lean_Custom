...
namespace QuantConnect.Algorithm.CSharp
{
    public class LiveStrategy : QCAlgorithm
    {
        ...
        {
            SetStartDate(2021, 06, 1);  //Set Start Date
            ...
            TimeRules.At(10, 30, 0), () => { PerformFilter(); });
            //PerformFilter();
            Schedule.On(DateRules.Every(DayOfWeek.Monday, DayOfWeek.Tuesday, DayOfWeek.Wednesday, DayOfWeek.Thursday, DayOfWeek.Friday),
        ...
        public void PerformFilter()
        {
            ...
            {
                var history = History(symb, 375 + 45 + 30, Resolution.Minute);
                yhigh = 0;
                thigh = 0;
                chigh = 0;
                clow = 100000;
            ...
            foreach (Symbol sym in FilterListSym)
            {
                ...
                {
                    Log("Stop Market Order Placed");
                    StopMarketOrder(sym, -qty, candlow[sym]);
                }
                else
                {
                    Log("Market Order Placed");
                    MarketOrder(sym, -qty);
                }
            }
        }
